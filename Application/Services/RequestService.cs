    ...
    : IRequestService
{
    public async Task<RequestDto> CreateRequestAsync(CreateRequestDto createRequestDto, int companyId, int adminId)
    {
        ...
        if (company == null) throw new ArgumentException($"Company with ID {companyId} does not exist.");
    ...
    public async Task<List<RequestDto>> GetAllRequestsByCompanyIdAsync(int companyId, int page, int size,
    ...
    public async Task<List<RequestDto>> GetAssignedRequestsAsync(int masterId, RequestStatus? status, int page,
        int size)
    {
    ...
    public async Task<Request> MasterAcceptRequestAsync(int requestId, int masterId)
    {
        var request = await requestRepository.GetRequestByIdAsync(requestId);
        if (request == null) throw new Exception("Equipment not found");
        if (request.Status != RequestStatus.Sent) throw new Exception($"Request {requestId} has already been sent");

        var master = await userRepository.GetByIdAsync(masterId);
        if (master == null || master.Role != UserRole.Master) throw new Exception($"Invalid master id {masterId}");
        var oldStatus = request.Status;
