...
public interface IRequestService
{
    ...
    Task<List<CompletedWorkPhotoDto>> UploadCompletedWorkPhotosAsync(int requestId, List<IFormFile> files, int userId,
        CancellationToken ct);

    Task<Request> MasterCompletedWorkAsync(int requestId, int masterId, List<string> photoUrls);

    Task<List<Request>> MasterGetAvailableRequestsAsync(int masterId, RequestStatus? status, int page = 1,
        int size = 10);
}